...
std::ostream& operator<<(std::ostream& f_out, const event_list f_value)
{
    ...
    switch (f_value)
    {
        case (event_list::init_ongoing):
        {
            s = "init_ongoing";
        }
        break;
        case (event_list::init_ended):
        {
            s = "init_ended";
        }
        break;
        case (event_list::run_ended):
        {
            s = "run_ended";
        }
        break;
...
std::ostream& operator<<(std::ostream& f_out, const state_list f_value)
{
    ...
    switch (f_value)
    {
        case (state_list::undefined):
        {
            s = "undefined";
        }
        break;
        case (state_list::init):
        {
            s = "init";
        }
        break;
...
{
    m_available_states.insert(
        std::pair<state_list, std::shared_ptr<Cstate>>(
            state_list::init,
            std::make_shared<Cstate_init>()
        )
    );
    m_available_states.insert(
        std::pair<state_list, std::shared_ptr<Cstate>>(
            state_list::run,
            std::make_shared<Cstate_run>()
        )
    );
